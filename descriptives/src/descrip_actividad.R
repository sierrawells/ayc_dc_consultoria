...
pacman::p_load(tidyverse, here, googledrive, scales, eulerr, ggtext)
...
caption_indesol <- "Fuente: Elaboración por Data Cívica a partir del Registro Federal de las OSC 2025"

caption_sat <- "Fuente: Elaboración por Data Cívica a partir de los directorios de donatarias autorizadas del SAT"
...
n_activo_indesol <- ultimo_anio_informe %>% 
  filter(activo) %>% 
  pull(n) %>% 
  sum()
...
ultimo_anio_informe_bar <- ultimo_anio_informe %>% 
  ...
  geom_label(aes(label = paste0(comma(n), " (", percent(perc, accuracy = 0.1), ")")),
             fontface = "bold", nudge_y = 0.01, fill = fondo, label.size = NA,
             family = labels_font) + 
  coord_flip() +
  labs(title = "Último año en que OSC del registro del INDESOL presentaron su informe anual",
       subtitle = paste0("De un total de ", comma(n_total_osc_indesol), " OSC en el registro, ",
                         "<span style = 'color:", pal[2], ";'>**",
                         "solo ", comma(n_activo_indesol), " son activas",
                         "</span>**"),
       caption = paste0(caption_indesol,
...
for (device in devices){
  ggsave(ultimo_anio_informe_bar,
         filename = paste0(paths$output, "/ultimo_anio_informe_indesol.", device),
         width = 12, height = 8)
  
  walk(paste0(paths$output, "/ultimo_anio_informe_indesol.", device),
...
anio_inscripcion_indesol_bar <- anio_inscripcion_indesol %>%
  ...
  geom_text(aes(label = paste0(comma(n), " (", percent(perc, accuracy = 0.1), ")")),
            fontface = "bold", nudge_y = -0.005, color = fondo, family = labels_font) +
  coord_flip() + 
...
for (device in devices){
  ggsave(anio_inscripcion_indesol_bar, 
         filename = paste0(paths$output, "/anio_inscripcion_indesol.", device),
         width = 12, height = 8)
  
  walk(paste0(paths$output, "/anio_inscripcion_indesol.", device),
       ~add_both_logos(.x, escala = 7))
}
...
for (device in devices){
  ggsave(perc_presentaron_lag_bar, 
         filename = paste0(paths$output, "/perc_indesol_lag.", device),
         width = 12, height = 8)
  
  walk(paste0(paths$output, "/perc_indesol_lag.", device),
       ~add_both_logos(.x, escala = 7))
}
...
for (device in devices){
  ggsave(anio_autorizacion_da_bar, 
         filename = paste0(paths$output, "/anio_autorizacion_da.", device),
         width = 12, height = 8)
  
  walk(paste0(paths$output, "/anio_autorizacion_da.", device),
       ~add_both_logos(.x, escala = 7))
}
...
for (device in devices){
  ggsave(perc_mantuvieron_da_bar, 
         filename = paste0(paths$output, "/perc_da_lag.", device),
         width = 12, height = 8)
  
  walk(paste0(paths$output, "/perc_da_lag.", device),
       ~add_both_logos(.x, escala = 7))
}
