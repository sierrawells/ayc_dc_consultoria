...
n_groups_donantes <- n_distinct(dist_donantes$donante_group)
...
graf_dist_donantes <- dist_donantes %>%
  ...
  tema +
  scale_fill_manual(values = rev(pal[1:n_groups_donantes])) +
  scale_color_manual(values = rev(pal[1:n_groups_donantes]), guide = "none") +
...
for (device in devices){
  ggsave(graf_dist_donantes,
         filename = paste0(paths$output, "dist_donantes.", device),
         width = 12, height = 10)
  
  walk(paste0(paths$output, "dist_donantes.", device),
...
for (device in devices){
  ggsave(graf_top_donantes,
         filename = paste0(paths$output, "top_donantes.", device),
         width = 12, height = 8)
  
  walk(paste0(paths$output, "top_donantes.", device),
       ~add_both_logos(.x, escala = 7))
}
...
for (device in devices){
  ggsave(filename = paste0(paths$output, "top_donatarios.", device),
         plot = graf_top_donatarios,
         width = 16, height = 12)
  
  walk(paste0(paths$output, "top_donatarios.", device),
       ~add_both_logos(.x, escala = 7))
}
