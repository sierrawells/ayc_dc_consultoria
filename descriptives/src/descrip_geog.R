...
pacman::p_load(tidyverse, here, googledrive, readxl, janitor, scales,
               ggtext, data.table, upstartr)
...
n_osc_sedes_por_estado <- caratula %>% 
  ...
  mutate(perc = n_osc / sum(n_osc, na.rm = TRUE),
         ...
         entidad_federativa = case_when(
           ...
           entidad_federativa == "Mexico" ~ "Estado de México",
           entidad_federativa == "Queretaro" ~ "Querétaro",
           ...
           str_detect(entidad_federativa, "Coahuila") ~ "Coahuila",
           str_detect(entidad_federativa, "Veracruz") ~ "Veracruz",
           str_detect(entidad_federativa, "Michoacan") ~ "Michoacán",
           T ~ entidad_federativa))

stopifnot(all(n_osc_sedes_por_estado$entidad_federativa %in% poblacion$nom_ent))
...
graf_n_osc_sedes_por_estado <- ggplot(n_osc_sedes_por_estado,
  ...
  geom_label(aes(label = paste0(n_osc, " (", percent(perc, accuracy = 0.1), ")")),
             hjust = -0.1, size = 3.5, fontface = "bold", family = labels_font,
             fill = "white", label.size = NA) +
  coord_flip() +
  labs(title = "¿Dónde se concentran las OSC?",
       subtitle = "Número de donatarias autorizadas con su domicilio fiscal en cada entidad federativa",
       x = "",
       y = "",
...
for (device in devices){
  ggsave(graf_n_osc_sedes_por_estado,
         filename = paste0(paths$output, "n_osc_sedes_por_estado.", device),
         width = 12, height = 8)
  
  walk(paste0(paths$output, "n_osc_sedes_por_estado.", device),
...
for (device in devices){
  ggsave(filename = paste0(paths$output, "perc_donativos_por_estado.", device),
         plot = graf_perc_donativos_por_estado,
         width = 12, height = 8)
  
  walk(paste0(paths$output, "perc_donativos_por_estado.", device),
       ~add_both_logos(.x, escala = 7))
}
...
for (device in devices){
  ggsave(filename = paste0(paths$output, "perc_donativos_municipios_rurales.", device),
         plot = graf_perc_donativos_municipios_rurales,
         width = 14, height = 10)
  
  walk(paste0(paths$output, "perc_donativos_municipios_rurales.", device),
       ~add_both_logos(.x, escala = 7))
}
